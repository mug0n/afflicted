...
Afflicted.applySpellEffects = function(self, target_id, spell_id)
    if (self.debugMode == true) then
        print(string.format("Applying effects from spell: %s to target %s.",  spell_id, target_id))
    end
    ...
    -- get the spell, stop if unrecognized
    ...
    for _, v in pairs(self.mobs[target_id]) do
        ...
        if (spell_id == v.spell_id or blocked_by:contains(v.spell_id)) then
            if (self.debugMode == true) then
                print(string.format("Spell %s on %s was blocked by %s.", spell_id, target_id, v.spell_id))
            end
            return
        end
    end

    -- remove any effects this spell is overwriting (spell.removes)
    local removes = self.mobs[target_id]:filter(function(a) return (spell.removes or T{}):contains(a.spell_id) end)
    for _, remove in pairs(removes) do
        if (self.debugMode == true) then
            print(string.format("Spell %s on %s overwrites %s.", spell_id, target_id, remove.effect_id))
        end
        self:removeEffect(target_id, remove.effect_id)
    end

    -- apply spell effect(s) -- spell.effect may or may not be a table
    local effects = type(spell.effect) == "table" and spell.effect or { spell.effect }
    for _, effect_id in pairs(effects) do
        self.mobs[target_id]:append({ spell_id = spell_id, effect_id = effect_id, expiration = os.time() + spell.duration })
    end

    -- sort by time left
    self.mobs[target_id]:sort(function(a, b) return a and b and a.expiration < b.expiration end)
end

Afflicted.removeEffect = function(self, target_id, effect_id)
    if (self.debugMode == true) then
        print(string.format("Removing effect: %s from target %s.",  effect_id, target_id))
    end

    local target = self:getTarget(target_id) or T{}
    local effect = target:find_if(function(a) return a.effect_id == effect_id end)

    -- remove effect
    if (effect ~= nil) then target[effect] = nil end
end
    ...
    if (param == "list") then
        ...
        for k, v in pairs(Afflicted.mobs) do
            ...
            for vk, vv in pairs(v) do
                ...
                print(header .. string.format(" spell=%s spell_id=%s effect_id=%s expiration=%s", spell.name, vv.spell_id, vv.effect_id, vv.expiration))
            end
        end
...
ashita.events.register("d3d_present", "afflicted_present", function()
    ...
    if (bit.band(entMgr:GetSpawnFlags(targetIndex), 0x10) == 0) then return end
        ...
        if (packet.type == 4) then
            for _, target in pairs(packet.targets) do
                ...
                if (action ~= nil and T{ 2, 236, 237, 252, 264, 268, 271, 277, 278 }:contains(action.message)) then
                    ...
                    Afflicted:applySpellEffects(target.id, packet.param)
                end
            end
        end
